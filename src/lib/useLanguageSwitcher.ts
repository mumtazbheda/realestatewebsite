...
export const GOOGLE_COOKIE_NAME = "googtrans";
...
export const useLanguageSwitcher = ({
  ...
  useEffect(() => {
    ...
    const existingLanguageCookieValue = cookies[GOOGLE_COOKIE_NAME];
  ...
  const switchLanguage = (lang: string) => () => {
    ...
    if (cfg && cfg.defaultLanguage === lang) {
      DeleteCookies({ name: GOOGLE_COOKIE_NAME }).then(() => {
        window.location.reload();
      });
    } else {
      setCookie(context, GOOGLE_COOKIE_NAME, "/auto/" + lang);
      window.location.reload();
    }
  };
